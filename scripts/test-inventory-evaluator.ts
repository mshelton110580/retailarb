...
// Order refunded (full OR partial), no return, no INR — compensated without a return; never needs-return
check("refund, no return, damaged in to_be_returned -> fair", evaluateUnitState({ condition_status: "damaged", inventory_state: "to_be_returned" }, [], [], true), "fair");
check("refund, no return, cracked in to_be_returned -> parts_repair", evaluateUnitState(badTBR, [], [], true), "parts_repair");
check("refund, no return, good in to_be_returned -> on_hand", evaluateUnitState(good, [], [], true), "on_hand");
check("refund, no return, bad on_hand -> parts_repair (never flagged)", evaluateUnitState(bad, [], [], true), "parts_repair");
check("no refund (flag false) keeps orphan rule", evaluateUnitState(bad, [], [], false), "to_be_returned");
check("refund does not override open return lifecycle", evaluateUnitState(badTBR, [R({ ebay_state: "RETURN_REQUESTED" })], [], true), "return_filed");
// open return
